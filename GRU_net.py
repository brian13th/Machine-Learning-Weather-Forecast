...
NAME = f"gru_real_5_500_{str(datetime.fromtimestamp(time.time()))}"
print(NAME)
...
# design network
model = Sequential()
model.add(GRU(10, input_shape=(1,32),return_sequences=True, kernel_initializer='uniform', activation='relu'))
model.add(GRU(10, input_shape=(1,32), activation='relu'))
...
# # gi.plt.plot(scalerMm_X.inverse_transform(target),'-b', scalerMm_X.inverse_transf